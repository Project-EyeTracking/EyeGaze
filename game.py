...
game_duration = 20  # 5 minutes in seconds
...
def game():
    ...
    out = cv2.VideoWriter(game_video_csv_file_path, fourcc, 20.0, (640,480))

    # CSV setup
    game_csv_file_path = CWD / "output" / f'Game_{movement_type}_{speed_choice}_{int(time.time())}.csv'
    game_csv_file_path.parent.mkdir(exist_ok=True)
    with open(game_csv_file_path, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Time", "X", "Y", "Speed_X", "Speed_Y"])
        ...
        while running:
            ...
            # Log data
            current_time = round(elapsed_time, 2)
            writer.writerow([current_time, obj_x, obj_y, speed_x, speed_y])
