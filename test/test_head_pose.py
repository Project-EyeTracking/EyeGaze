import cv2
import numpy as np
from batch_face import RetinaFace, SixDRep, Timer, draw_landmarks, load_frames_rgb
...
if __name__ == "__main__":
    ...
    all_faces = detector(
        frames, batch_size=batch_size, return_dict=True, threshold=threshold, resize=0.5
    )
    head_poses = head_pose_estimator(
        all_faces, frames, batch_size=batch_size, update_dict=True, input_face_type="dict"
    )
    print(f"{all_faces[0]=}")
