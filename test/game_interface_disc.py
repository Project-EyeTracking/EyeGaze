...
# Set constant speed (in pixels per jump)
CONSTANT_SPEED = 100
...
def setup_screen():
    ...
    # Arrange movement type buttons in one row:
    ...
    while running:
        screen.fill(WHITE)
        title = font.render("Select Movement", True, BLACK)
...
def game(camera_id=0, frame_width=640, frame_height=480, game_video_file_path=None, csv_path=None):
    ...
    # For Diagonal mode, initialize parameters to interpolate along the path.
    ...
    # Use constant speed for all movements.
    speed_x = CONSTANT_SPEED
    speed_y = CONSTANT_SPEED

    # Mode-specific variables for other modes.
    ...
    # Set up a timer so that the position is updated only every 1 seconds.
    ...
    while running:
        ...
        # Update position only if 1 seconds have elapsed since the last jump.
        ...
        if current_time - last_move_time >= 1:
            ...
            if movement_type == "Horizontal":
                ...
                # Compute the total length of the diagonal path.
                L = math.sqrt((WIDTH - obj_width) ** 2 + (HEIGHT - obj_height) ** 2)
                dt = CONSTANT_SPEED / L  # parameter increment per jump
                ...
                if diagonal_t >= 1:
                    ...
                    diagonal_t = 0
                    diagonal_direction = 1
                # Interpolate along the line from top-right to bottom-left.
                # Top right (t=0): (WIDTH - obj_width, 0)
                # Bottom left (t=1): (0, HEIGHT - obj_height)
                obj_x = (WIDTH - obj_width) * (1 - diagonal_t)
                obj_y = (HEIGHT - obj_height) * diagonal_t
            ...
            # Ensure that the displacement is at least 30 pixels.
            dx = obj_x - old_x
            dy = obj_y - old_y
            distance = math.hypot(dx, dy)
            if distance < 30 and distance > 0:
                factor = 30 / distance
                obj_x = old_x + dx * factor
                obj_y = old_y + dy * factor

            # Reset the timer for the next jump.
            last_move_time = current_time

        # Draw the interactive icon (or fallback to a rectangle)
        if icon_image:
        ...
        info_text = font.render(
            f"Movement: {movement_type} | Speed: Constant({CONSTANT_SPEED}) | Time Left: {int(time_remaining)}s",
            True,
            BLACK,
        )
